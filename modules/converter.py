...
def currency_converter_function(amount, source_currency, target_currency):
    ...
    if (
        source_currency in processed_currency_list
        and target_currency in processed_currency_list
    ):
        # Check if at least one of the currencies is volatile
        # Verificar si al menos una de las monedas es volátil
        if (
            source_currency in volatile_currency_list
            or target_currency in volatile_currency_list
        ):
            volatile = True
        ...
        # If currencies are not in the lists,
        # do not execute, and the result is an empty string
        # Si las monedas no están en las listas,
        # no ejecutar y el resultado es una cadena vacía
        execute = False
        result = ""
